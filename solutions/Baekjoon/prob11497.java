...
public class prob11497 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int test_case = 1; test_case <= T; test_case++) {
            ...
            int ans = binarySearch(min, max, arr);
            sb.append(ans).append("\n");
        }
        System.out.println(sb.toString());
    }

    private static int binarySearch(int min, int max, int[] targetArr) {
        int mid = 0;
        int left = min;
        int right = max;
        while (true) {
            ...
            boolean flag = isPossibleValue(mid, targetArr);
            if (flag) {
                right = mid - 1;
            } else {
                left = mid + 1;
